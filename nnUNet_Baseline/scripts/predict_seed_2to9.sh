...
for SEED_ID in {1..9}; do
  echo "=== Predicting seed_${SEED_ID} ==="
  export nnUNet_results="$PROJECT_ROOT/data/nnUNet_results/Dataset001_nnunet/seed_${SEED_ID}"

  OUTPUT_DIR="$PROJECT_ROOT/data/nnUNet_results/Dataset001_nnunet/preds/seed_${SEED_ID}"
  mkdir -p "$OUTPUT_DIR"
